...
def loss_after_odors(W_rec: torch.Tensor, h_bar_ff: torch.Tensor, threshold_mult, lambda_corr, lambda_i, lambda_mu, lambda_var, lambda_sp, do_print):   
    R_new = compute_piriform_response(h_bar_ff, W_rec, threshold_mult)
    loss = loss_fn(R_new, lambda_corr, lambda_i, lambda_mu, lambda_var, lambda_sp, do_print)
...
# 1e-3 sets weights to 0 - do 1e-4
lambda_corr, lambda_i, lambda_mu, lambda_var, lambda_sp = 1, 0, 0, 0, 9e-4
...
def train_model():
    ...
    loss_fn(R_initial, lambda_corr, lambda_i, lambda_mu, lambda_var, lambda_sp)
    ...
    for i in range(epochs_inner):
        ...
        if (i % 100 == 0):
            print(f"Epoch {i}: \t", end="")
            do_print = True
        loss, R_trained = loss_after_odors(W_trained, hbar_ff, threshold_multiplier, lambda_corr, lambda_i, lambda_mu, lambda_var, lambda_sp, do_print)
...
def save_realization(i):
    ...
    with torch.no_grad():
        ...
        sp_odor_trained = sparsity_per_odor(R_trained).detach().cpu()

        ax[0].bar(x_p, sp_odor_initial[novel_inds].detach().cpu(), width=width, label="Novel", color="b")
        ax[0].bar(x_p + width, sp_odor_initial[familiar_inds].detach().cpu(), width=width, label="Familiar", color="g")
        ax[0].legend()
        ax[0].set_xticks([])
        ax[0].set_title("Initial")
        ax[1].bar(x_p, sp_odor_trained[novel_inds].detach().cpu(), width=width, label="Novel", color="b")
        ax[1].bar(x_p + width, sp_odor_trained[familiar_inds].detach().cpu(), width=width, label="Familiar", color="g")
        ax[1].legend()
        ax[1].set_xticks([])
        ax[1].set_title("Trained")

        fig.suptitle("E->I: Sparsity Per Odor")
        fig.supxlabel("Odor")
        fig.supylabel("Sparsity")
        fig.savefig(f"{path}/ie_sp.png")

        W2 = W_trained.detach().cpu().numpy()
        W_ie = W2[num_e:, :num_e]
        fig = plt.figure()
        plt.plot(np.sum(W_ie, axis=0), "bo")
        plt.title("Weighted outdegree of E")
        fig.savefig(f"{path}/ie_outdegree.png")
        fig = plt.figure()
        plt.plot(np.sum(W_ie, axis=1), "bo")
        plt.title("Weighted indegree of I")
        fig.savefig(f"{path}/ie_indegree.png")
...
# plt.plot(torch.mean(R_initial.detach().cpu(), dim=1)[num_e:])
